    ...
    <div class="container display-flex flex-justify-between flex-align-center">
      ...
      <nav class="top-nav">
        ...
        <?php
          ...
          $cart_count_hero = 0;
          foreach ($cart_items_hero as $item) { $cart_count_hero += (int)($item['amount'] ?? 0); }
        ?>
        <a href="confirmation.php">Cart (<?= $cart_count_hero ?>)</a>
      </nav>
      <button class="hero-nav-toggle" aria-label="Open menu" onclick="document.querySelector('.hero-nav .top-nav').classList.toggle('open')">☰</button>
    </div>
  </header>
